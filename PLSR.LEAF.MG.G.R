...
df_reordered_leaf<-read_excel("~/Desktop/spectra/DF/df_reordered_leaf.xlsx")
...
pls_rec_totalanth_mgg <- make_pls_rec("mg_gerry")
...
pls_rec_myricetin_mgg <- make_pls_rec("myricetin_mg_g")
pls_rec_quercetin_mgg <- make_pls_rec("quercetin_mg_g")
pls_rec_kaempferol_mgg <- make_pls_rec("kaempferol_mg_g")

rec_list_flav <- list(
  pls_rec_myricetin_mgg,
  pls_rec_quercetin_mgg,
  pls_rec_kaempferol_mgg
)
...
#*# Run only kaempferol
rec_list_flav <- list(pls_rec_kaempferol_mgg)
pred_list_flav <- c("kaempferol_mg_g")
...
flav_leaf_plot <- ggplot(
  ...
  coord_cartesian(clip = "off") +
  labs(
    title = "Leaf Spectra: Actual vs Predicted Flavonol Composition (mg/gerry) for Testing Dataset",
    x = "Observed value",
    y = "Predicted value",
    color = "Date"
  ) +
...
tannin_plot <- df_test_leaf %>%
  mutate(
    param = recode(
      param,
      "total_tannin_mgg" = "Total tannins"
    )
  ) %>%
  mutate(
    param = factor(
...
tannin_data_leaf <- df_test_leaf %>%
  ...
  filter(param == "Total tannins (mg/gerry)")
...
combined_plot_data <- bind_rows(
  ...
  mutate(
    dataset = factor(dataset, levels = c("Berry", "Leaf")),
    param = factor(param, levels = c("Total tannins (mg/gerry)")),
    date = factor(
      date,
