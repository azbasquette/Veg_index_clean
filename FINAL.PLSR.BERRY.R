...
# --- Recipe function: spectra only / metadata only / combined ---
...
# --- Primary chemistry response variables: combined spectra + metadata ---
pls_rec_brix_combined <- make_pls_rec("brix")
pls_rec_ph_combined   <- make_pls_rec("p_h")
pls_rec_ta_combined   <- make_pls_rec("ta_g_l_tartaric_acid")

rec_list_combined <- list(
  pls_rec_brix_combined,
  pls_rec_ph_combined,
  pls_rec_ta_combined
)

pred_list_combined <- c(
  "brix",
  "p_h",
...
# --- Sanity checks ---
rec_list_combined[[1]]$term_info %>%
  dplyr::filter(role == "predictor") %>%
  dplyr::pull(variable)

# --- PLSR loop ---
for (i in seq_along(rec_list_combined)) {
  cat("In progress:", i, "out of", length(rec_list_combined), "\n")
  
  pls_rec <- rec_list_combined[[i]]
  target  <- pred_list_combined[[i]]
...
label_df <- tibble::tibble(
  param = c("Brix", "pH", "TA (g/L tartaric acid)"),
  panel_label = c("(A)", "(B)", "(C)")
)

#*******************
avptest <- ggplot(plot_df, aes(truth, .pred, color = date)) +
  geom_point() +
  geom_smooth(
    aes(color = NULL),
    method = "lm",
  ...
  labs(
    title = "Berry Spectra: Actual vs Predicted Brix, pH, and TA for Testing Dataset",
    x = "Observed value",
    y = "Predicted value",
    color = "Date"
  ) +
...
avptest_anth_combined <- ggplot(plot_df, aes(truth, .pred, color = date)) +
  ...
  labs(
    title = "Berry Spectra: Actual vs Predicted Anthocyanins (mg/berry) for Testing Dataset ",
    x = "Observed value",
    y = "Predicted value",
    color = "Date"
  ) +
...
# --- Response variables ---
pls_rec_myr_mgb <- make_pls_rec("myricetin_mg_b")
pls_rec_que_mgb <- make_pls_rec("quercetin_mg_b")
pls_rec_kae_mgb <- make_pls_rec("kaempferol_mg_b")
...
flavonol_plot <- df_test %>%
  mutate(
    param = recode(
      param,
      "myricetin_mg_b" = "Myricetin (mg/b)",
      "quercetin_mg_b"  = "Quercetin (mg/b)",
      "kaempferol_mg_b" = "Kaempferol (mg/b)"
    )
  ) %>%
  filter(
    param %in% c(
...
label_df <- tibble::tibble(
  param = factor(
    c("Myricetin (mg/b)", "Quercetin (mg/b)", "Kaempferol (mg/b)"),
    levels = c("Myricetin (mg/b)", "Quercetin (mg/b)", "Kaempferol (mg/b)")
  ),
  label = c("(A)", "(B)", "(C)")
)
...
flavonol_berry_plot <- ggplot(
  ...
  coord_cartesian(clip = "off") +
  labs(
    title = "Berry Spectra: Actual vs Predicted Myricetin, Quercetin, and Kaempferol (mg/berry) for Testing Dataset",
    x = "Observed value",
    y = "Predicted value",
    color = "Date"
  ) +
...
rec_list_tannin <- list(pls_rec_tannin_mgb)
pred_list_tannin <- c("total_tannin_mgb")
...
tannin_plot <- df_test_berry %>%
  filter(param == "total_tannin_mgb") %>%
  mutate(
    param = recode(
      param,
      "total_tannin_mgb" = "Total Tannin (mg/berry)"
    )
  )
...
tannin_test_plot <- ggplot(tannin_plot, aes(truth, .pred, color = date)) +
  ...
  coord_cartesian(clip = "off") +
  labs(
    title = "Actual vs Predicted Total Tannin (mg/berry) for testing dataset",
    x = "Observed value",
    y = "Predicted value",
    color = "Date"
  ) +
...
tannin_data_berry <- df_test_berry %>%
  ...
  filter(param %in% c("Total tannins (mg/berry)"))